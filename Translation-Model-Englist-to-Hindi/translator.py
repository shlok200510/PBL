...
def beam_search(sentence, model, src_field, src_tokenizer, trg_field, trg_vcb_sz, k, max_ts=50, device="cpu"):
    ...
    for ts in range(max_ts):
        if ts == 0:
            ...
            #print("Raw top indices:", topk.indices[0][:5])
            ...
            if eos in seq_id[:, ts + 1]:
                mask = (seq_id[:, ts + 1] == eos)
                eos_probs = seq_prob[mask]
                eos_seqs = seq_id[mask]

                for prob, seq in zip(eos_probs, eos_seqs):
                    trans_store[prob.item()] = seq
            ...
            all_comb_idx = torch.tensor([(x, y) for x in range(store_seq_id.shape[-2]) for y in range(trg_vcb_sz)]).to(device)
            topk = torch.topk(all_comb, dim=-1, k=k)
            top_seq_id = all_comb_idx[topk.indices.squeeze()]
            top_seq_prob = topk.values
            seq_id = torch.empty(size=(k, ts + 2), dtype=torch.long)
            seq_id[:, :ts + 1] = torch.tensor([store_seq_id[i.tolist()].tolist() for i, y in top_seq_id]).to(device)
            seq_id[:, ts + 1] = torch.tensor([y.tolist() for i, y in top_seq_id]).to(device)
